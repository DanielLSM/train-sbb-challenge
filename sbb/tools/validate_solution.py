# https://github.com/crowdAI/train-schedule-optimisation-challenge-starter-kit/tree/master/utils
import requests
import translate
import json
...
solution = os.path.join(parentdir, 'sample_files',
                        "sample_scenario_solution.json")
...
def do_loesung_validation(scenario, solution):
    ...
    scenario_content = translate.translate(scenario_content,
                                           translate.translate_to_ger)

    scenario_file = {"verkehrsplan": StringIO(json.dumps(scenario_content))}
    upload_response = requests.post(
        SCENARIO_UPLOAD_ENDPOINT, files=scenario_file, auth=AUTH)
    ...
    solution_content = translate.translate(solution_content,
                                           translate.translate_to_ger)
    solution_file = {"loesung": StringIO(json.dumps(solution_content))}
    validation_response = requests.post(
        SOLUTION_VALIDATION_ENDPOINT, files=solution_file, auth=AUTH)
    print(f"validation finished with status {validation_response.status_code}")

    response = validation_response.json()
    validation_result = translate.translate(response,
                                            translate.translate_to_eng)
    validation_result = translate.translate_message_word_for_word(
        validation_result)
...
if __name__ == "__main__":
    ...
    warnings = [
        x for x in validation_result["business_rules_violations"]
    ...
    if len(errors) > 0:
        print(
            f"the solution has {len(errors)} errors. It will not be accepted as a feasible solution. "
        ...
        print()
        print(
            f"the solution has {len(warnings)} warnings. It will be accepted as a feasible solution. "
        )
        if validation_result['objective_value'] > 0.0:
            print(
                f"However, it will incur {validation_result['objective_value']} penalty points in the grader."
            )
